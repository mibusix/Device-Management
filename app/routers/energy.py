...
from app.models import Device, EnergyRecord
...
def calculate_energy(data: EnergyCreate, db: Session = Depends(get_db)):
    ...
    record = EnergyRecord(
        device_id=data.device_id, power=data.power,
        runtime_hours=data.runtime_hours, energy_kwh=energy_kwh,
        record_date=date.fromisoformat(data.record_date), notes=data.notes,
    )
    db.add(record)
    db.commit()
...
def list_records(db: Session = Depends(get_db)):
    ...
    return [{
        ...
        "power": r.power, "runtime_hours": r.runtime_hours,
        "energy_kwh": r.energy_kwh,
        "record_date": str(r.record_date), "notes": r.notes,
    } for r in records]
